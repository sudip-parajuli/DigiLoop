...
export async function POST(req: NextRequest) {
  try {
    ...
    const toEmail = process.env.CONTACT_EMAIL || "hello@digiloop.com.np";
    ...
    if (resendKey && resendKey !== "your_resend_key") {
      ...
      await resend.emails.send({
        from: "DigiLoop Contact <noreply@digiloop.com.np>",
        to: [toEmail],
        replyTo: email,
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; background: #f8f7f4; border-radius: 12px;">
            <div style="background: #1A1A2E; padding: 24px; border-radius: 8px; margin-bottom: 24px;">
              <h1 style="color: white; font-size: 24px; margin: 0;">DigiLoop — New Enquiry</h1>
            </div>
            ...
            <p style="margin-top: 24px; color: #6B6B6B; font-size: 12px; text-align: center;">DigiLoop · Kathmandu, Nepal · digiloop.com.np</p>
          </div>
        `,
      });
    } else {
